...
#if UE_4_25_OR_LATER
	auto assetName = FName(folderAssetPath);
#else
	auto assetName = FName(*folderAssetPath);
#endif
	auto folderAsset = AssetRegistryModule->Get().GetAssetByObjectPath(assetName);
...
#if UE_4_25_OR_LATER
		auto oldPath = FSoftObjectPath(objectPath);
#else
		auto oldPath = FSoftObjectPath(objectPath.ToString());
#endif
		TArray<FAssetRenameData> assetsToRename = { {oldPath, FSoftObjectPath(AssetPath) } };
