...
struct AkAcousticSurface
{
	...
	/// Occlusion value to set when modeling sound transmission through geometry. Transmission is modeled only when the sound emitted enables diffraction and there is no direct line of sight from the emitter to the listener.
	/// If more that one surface is between the emitter and the listener, the maximum of each surface's occlusion value is used. If the emitter and listener are in different rooms, then the rooms' WallOcclusion is taken into account.
...
struct AkRoomParams
{
	...
	AkRoomParams(const AkRoomParams& in_rhs)
		...
		, WallOcclusion(in_rhs.WallOcclusion)
	...
	/// Occlusion level to set when modeling transmission through walls. Transmission is modeled only when the sound emitted enables diffraction and there is no direct line of sight from the emitter to the listener.
